...
func TestSendReadRequestRoundTrip(t *testing.T) {
	...
	go func() {
		errCh <- daemon.SendRequest(client, req)
	}()

	got, err := daemon.ReadRequest(server)
	if err != nil {
...
func TestSendReadResponseRoundTrip(t *testing.T) {
	...
	resp := &daemon.Response{OK: true, Data: "process started"}

	errCh := make(chan error, 1)
	go func() {
		errCh <- daemon.SendResponse(server, resp)
	}()

	got, err := daemon.ReadResponse(client)
	if err != nil {
...
func TestReadRequestMalformedJSON(t *testing.T) {
	...
	_, err := daemon.ReadRequest(server)
	if err == nil {
...
func TestReadResponseMalformedJSON(t *testing.T) {
	...
	_, err := daemon.ReadResponse(server)
	if err == nil {
...
func TestRequestOmitEmptyArgs(t *testing.T) {
	...
	t.Run("populated args included", func(t *testing.T) {
		req := daemon.Request{Action: "serve", Args: map[string]any{"name": "app"}}
		data, err := json.Marshal(req)
		if err != nil {
			t.Fatalf("json.Marshal failed: %v", err)
		}
...
func TestResponseOmitEmptyFields(t *testing.T) {
	...
	t.Run("error response omits data", func(t *testing.T) {
		resp := daemon.Response{OK: false, Error: "something failed"}
		data, err := json.Marshal(resp)
		if err != nil {
			t.Fatalf("json.Marshal failed: %v", err)
		}
